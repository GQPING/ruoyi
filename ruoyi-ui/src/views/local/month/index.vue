    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['local:month:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="monthList" @selection-change="handleSelectionChange" border>
      ...
      <el-table-column label="账单月份" align="center" prop="month" width="106">
        <template slot-scope="scope">
...
<script>
import { listMonth, delMonth } from "@/api/local/month";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除月度账单编号为"' + ids + '"的数据项？').then(function() {
        return delMonth(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('local/month/export', {
        ...this.queryParams
